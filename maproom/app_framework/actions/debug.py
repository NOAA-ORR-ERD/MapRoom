from ..action import MafAction, MafSubAction, MafActionRadioMixin
from .. import errors


class debug_text_counting(MafAction):
...
class debug_text_last_digit(MafActionRadioMixin, MafAction):
    def calc_name(self, action_key):
        return action_key.replace("_", " ").title()
...
class debug_text_last_digit_dyn(MafAction):
    def init_from_editor(self, action_key):
...
class debug_text_size(MafAction):
    def init_from_editor(self, action_key):
        self.counts = list(range(5, 25, 5))
...
class debug_generated(MafSubAction):
