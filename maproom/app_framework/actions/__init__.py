...
from ..action import MafAction, MafNameChangeAction, MafListAction, MafRadioAction
...
class new_window(MafAction):
    name = "New Window"

    def perform(self, event=None):
        frame = MafFrame(None, wx.GetApp().app_blank_uri)
        frame.Show()


class new_blank_file(MafAction):
...
class new_file_from_template(MafListAction):
...
class quit(MafAction):
    name = "Quit"

    def perform(self, action_key):
        wx.GetApp().quit()

class undo(MafNameChangeAction):
...
class redo(MafNameChangeAction):
    def calc_name(self, action_key):
...
class delete_selection(MafAction):
...
class prefs(MafAction):
    name = "Preferences"

    def perform(self, action_key):
        wx.GetApp().show_preferences_dialog(self.editor.frame, self.editor.ui_name)

class about(MafAction):
    name = "About"

    def perform(self, action_key):
        wx.GetApp().show_about_dialog()

class prev_line(MafAction):
    name = "Previous Line"

    def perform(self, action_key):
        print("Up!")

class next_line(MafAction):
    name = "Next Line"

    def perform(self, action_key):
        print("Down!")

class prev_char(MafAction):
    name = "Previous Char"

    def perform(self, action_key):
        print("Left!")

class next_char(MafAction):
    name = "Next Char"

    def perform(self, action_key):
        print("Right!")

class show_toolbar(MafRadioAction):
    name = "Show Toolbar"
...
class raise_exception(MafAction):
    name = "Raise Exception"

    def perform(self, action_key):
        val = int("this will raise a ValueError")

class test_progress(MafAction):
...
class show_debug_log(MafAction):
    name = "View Error Log"

    def perform(self, action_key):
        show_logging_frame()

class widget_inspector(MafAction):
    name = "View Widget Inspector"

    def perform(self, action_key):
        wx.lib.inspection.InspectionTool().Show()

class show_focus(MafAction):
