...
def find_action_factory_in_module(mod_root, action_key):
    ...
    For example, if the module is "maproom.app_framework.actions" and the action
    key is "file_save_as", the lookup order will be:

        maproom.app_framework.actions.file_save_as.py
        maproom.app_framework.actions.file_save.py
        maproom.app_framework.actions.file.py
...
class MafNameChangeAction(MafAction):
    def sync_menu_item_from_editor(self, action_key, menu_item):
        MafAction.sync_menu_item_from_editor(self, action_key, menu_item)
        name = self.calc_name(action_key)
        menu_item.SetText(name)


class MafActionRadioMixin:
    def append_to_menu(self, menu, id, action_key):
...
class MafRadioAction(MafActionRadioMixin, MafAction):
...
class MafListAction(MafActionListMixin, MafAction):
    def sync_menu_item_from_editor(self, action_key, menu_item):
...
class MafPersistentDictAction(MafListAction):
...
class MafRadioListAction(MafActionListMixin, MafActionRadioMixin, MafAction):
