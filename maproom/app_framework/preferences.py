...
class MafApplicationPreferences(MafPreferences):
    ui_name = "General"
    ...
    def set_defaults(self):
        MafPreferences.set_defaults(self)
        self.num_open_recent = 15
        self.confirm_before_close = True


class MafEditorPreferences(MafPreferences):
    display_order = [
        ("text_font", "wx.Font"),
...
def find_application_preferences(prefs_module):
    ...
    prefs_cls = MafApplicationPreferences
    for name, obj in inspect.getmembers(mod):
        if inspect.isclass(obj) and MafApplicationPreferences in obj.__mro__[1:]:
            prefs_cls = obj
            break
...
def find_editor_preferences(prefs_module):
    mod = importlib.import_module(prefs_module)
    prefs_cls = MafEditorPreferences
    for name, obj in inspect.getmembers(mod):
        if inspect.isclass(obj) and MafEditorPreferences in obj.__mro__[1:]:
            prefs_cls = obj
            break
    return prefs_cls(prefs_module)
