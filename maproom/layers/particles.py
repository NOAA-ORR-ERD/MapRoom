...
But there is a bunch of stuff in PointLayer this we don't need here
"""
import os
import os.path
import time
...
class ParticleLayer(ProjectedLayer):
    ...
    def insert_point_at_index(self, point_index, world_point, z, color, state, add_undo_info):
        ...
        # insert it into the point_renderer (by simply rebuilding it)
        # we don't update the point_renderer if not adding undo info, because that means we are undoing or redoing
        # and the point_renderer for all affected layers will get rebuilt at the end of the process
        if (add_undo_info):
    ...
    def create_renderer(self, renderer):
        ...
        """
        if self.points != None and renderer.point_renderer == None:
    ...
    def render_projected(self, renderer, w_r, p_r, s_r, layer_visibility, layer_index_base, pick_mode=False):
        ...
        if (renderer.point_renderer != None):
            ...
            # the labels
            if (renderer.label_set_renderer != None and layer_visibility["labels"] and renderer.point_renderer.vbo_point_xys != None):
                renderer.label_set_renderer.render(-1, pick_mode, s_r,
                                               renderer.MAX_LABEL_CHARACTERS, self.points.z,
                                               renderer.point_renderer.vbo_point_xys.data,
